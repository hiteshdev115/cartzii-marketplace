...
export function CartItem({ item }: CartItemProps) {
  ...
  return (
    <div className="flex gap-4 p-4 border-b border-gray-100">
      <Link href={productHref} className="relative w-24 h-24 md:w-32 md:h-32 rounded-xl overflow-hidden shrink-0">
        <Image src={item.product.images[0]} alt={item.product.name} fill className="object-cover" sizes="128px" />
      </Link>
      <div className="flex-1 min-w-0">
        <Link href={productHref}>
          <h3 className="font-semibold text-slate-900 hover:text-primary transition-colors">{item.product.name}</h3>
        </Link>
